...
// canónica) no debe reaparecer. El ACEPTO de mandados envía UNA sola
// comunicación interactiva (buildMandadoAssignmentInteractive); los
// restaurantes conservan la plantilla confirmacion_repartidor + deliveryNotes.
test("el webhook envía a mandados una única comunicación y conserva el flujo de restaurantes", () => {
  ...
  assert.match(source, /buildMandadoAssignmentInteractive/);
  assert.match(source, /sendWhatsAppInteractiveMessage/);
  assert.match(source, /sendDriverConfirmation/);
