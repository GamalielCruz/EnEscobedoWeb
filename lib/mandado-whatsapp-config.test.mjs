...
test("mandado__cliente envia 3 variables de cuerpo en el orden aprobado: nombre, direccion (tras el 📍), folio", () => {
  const result = buildMandadoTemplateComponents({
    templateName: "mandado__cliente",
...
test("mandado__destinatario recibe el aviso final o el NIP en su única variable y omite botones", () => {
  const result = buildMandadoTemplateComponents({
    templateName: "mandado__destinatario",
    bodyParameters: ["Tu paquete ya llegó. Ya puedes recibirlo."],
  });
  assert.deepEqual(result.buttonComponents, []);
  assert.deepEqual(result.bodyParameters, ["Tu paquete ya llegó. Ya puedes recibirlo."]);
  assert.equal(MANDADO_WHATSAPP_TEMPLATES.recipientOnTheWay.hasButtons, false);
  assert.deepEqual(MANDADO_WHATSAPP_TEMPLATES.recipientOnTheWay.bodyVariables, ["recipientMessage"]);
});
...
test("mandado__cliente rechaza botones inventados", () => {
  assert.throws(
    () => buildMandadoTemplateComponents({
      templateName: "mandado__cliente",
      bodyParameters: ["Juan", "5 de febrero #64", "#240952"],
...
test("cada plantilla exige exactamente sus variables aprobadas", () => {
  // mandado__destinatario requiere exactamente una variable.
  assert.throws(
    () => buildMandadoTemplateComponents({
      templateName: "mandado__destinatario",
      bodyParameters: [],
    }),
    /requiere 1 variables/
  );
  assert.throws(
    () => buildMandadoTemplateComponents({
      templateName: "mandado__destinatario",
