...
test("TEST H: Mandado $44 — Tu envío $44, Total pagado $54", () => {
  ...
  assert.ok(!message.includes("Responde ACEPTO"), "NO contiene instrucción de texto");
});
...
// ============================================================
// TEST D2: Botón Aceptar mandado existe
// ============================================================
test("TEST D2: Botón Aceptar mandado con payload ACEPTO", () => {
  const button = { type: "reply", id: "ACEPTO", title: "Aceptar mandado" };
  assert.equal(button.id, "ACEPTO", "Payload del botón es ACEPTO");
  assert.equal(button.title, "Aceptar mandado", "Título del botón");
  assert.ok(button.title.length <= 20, "Título ≤ 20 caracteres");
});

// ============================================================
// TEST E2: El webhook procesa ACEPTO del botón igual que texto
// ============================================================
test("TEST E2: button_reply.id=ACEPTO → mismo flujo que texto ACEPTO", () => {
  // Simula parseButtonActionPayload + normalizeDriverActionToken
  function normalizeText(text) {
    return text.toUpperCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim();
  }
  function normalizeDriverActionToken(text) {
    return normalizeText(text).replace(/_/g, ' ');
  }
  function parseButtonActionPayload(rawValue) {
    const trimmed = String(rawValue || '').trim();
    if (!trimmed) return { action: '', orderId: null };
    const [rawAction, ...rawOrderIdParts] = trimmed.split('|');
    return { action: normalizeDriverActionToken(rawAction), orderId: rawOrderIdParts.join('|').trim() || null };
  }

  // Botón ACEPTO
  const buttonResult = parseButtonActionPayload('ACEPTO');
  assert.equal(buttonResult.action, 'ACEPTO');

  // Texto ACEPTO (flujo actual)
  const textResult = parseButtonActionPayload('ACEPTO');
  assert.equal(textResult.action, 'ACEPTO');

  // Son idénticos
  assert.equal(buttonResult.action, textResult.action, 'Mismo resultado que texto ACEPTO');
});
