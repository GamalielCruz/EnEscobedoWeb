...
import { classifyAssignmentOutcome, validateAssignment } from "./dispatch-validation.ts";
...
// ──────────────────────────────────────────────────────────────────────
// classifyAssignmentOutcome: decisión por el estado REAL de Sanity tras un
// intento de asignación fallido/conflictivo (idempotencia del 409).
// ──────────────────────────────────────────────────────────────────────

test("classify: pedido asignado a mí tras un conflicto = éxito idempotente", () => {
  const outcome = classifyAssignmentOutcome(
    { _id: "o1", driverId: "d1", dispatchStatus: "accepted", offeredToRef: "d1" },
    "d1"
  );
  assert.equal(outcome.kind, "assigned_to_me");
});

test("classify: pedido asignado a otro repartidor = no asignar, no liberar", () => {
  const outcome = classifyAssignmentOutcome(
    { _id: "o1", driverId: "d-otro", dispatchStatus: "accepted", offeredToRef: "d-otro" },
    "d1"
  );
  assert.equal(outcome.kind, "assigned_to_other");
  assert.equal(outcome.otherDriverId, "d-otro");
});

test("classify: oferta sigue vigente para mí = se puede reintentar o liberar solo por validación", () => {
  const outcome = classifyAssignmentOutcome(
    { _id: "o1", driverId: null, dispatchStatus: "offered", offeredToRef: "d1" },
    "d1"
  );
  assert.equal(outcome.kind, "still_offered");
});

test("classify: oferta liberada/expirada/rechazada = no asignar", () => {
  // Pedido devuelto a la cola (waiting_for_driver) sin offeredTo
  assert.equal(
    classifyAssignmentOutcome({ _id: "o1", driverId: null, dispatchStatus: "waiting_for_driver", offeredToRef: null }, "d1").kind,
    "offer_released"
  );
  // La oferta quedó ofrecida a OTRO repartidor
  assert.equal(
    classifyAssignmentOutcome({ _id: "o1", driverId: null, dispatchStatus: "offered", offeredToRef: "d-otro" }, "d1").kind,
    "offer_released"
  );
  // Pedido inexistente
  assert.equal(classifyAssignmentOutcome(null, "d1").kind, "order_missing");
});

test("classify: pedido sin repartidor ni oferta pero en estado terminal = no asignar", () => {
  assert.equal(
    classifyAssignmentOutcome({ _id: "o1", driverId: null, dispatchStatus: "completed", offeredToRef: null }, "d1").kind,
    "offer_released"
  );
});

test("classify: oferta vencida = no asignar (offer_released)", () => {
  const now = Date.now();
  const expired = { _id: "o1", driverId: null, dispatchStatus: "offered", offeredToRef: "d1", deliveryOfertaExpiresAt: new Date(now - 1000).toISOString() };
  assert.equal(classifyAssignmentOutcome(expired, "d1", now).kind, "offer_released");
  // La misma oferta sin vencer sigue vigente
  const valid = { ...expired, deliveryOfertaExpiresAt: new Date(now + 60_000).toISOString() };
  assert.equal(classifyAssignmentOutcome(valid, "d1", now).kind, "still_offered");
});
